...
function emptyPlan(): WeekPlan {
  return Object.fromEntries(
    DAYS.map((day) => [
      day,
      Object.fromEntries(MEALS.map((meal) => [meal, [] as string[]])),
    ]),
  ) as WeekPlan;
}
...
let week = $state<WeekPlan>(
  sanitize(loadPersisted<WeekPlan>(KEY, emptyPlan())),
);
...
export const planner = {
  get count() {
    return DAYS.reduce(
      (total, day) =>
        total + MEALS.reduce((sum, meal) => sum + week[day][meal].length, 0),
      0,
    );
  },
