...
import type {
  RawRecipeDetail,
  RawSearchResponse,
} from "$lib/api/spoonacular.types";
...
async function fetchRecipes(): Promise<RawRecipeDetail[]> {
  ...
  if (!apiKey) {
    console.warn(
      "SPOONACULAR_API_KEY is not set — serving the bundled fallback recipes.",
    );
...
function matchesFilter(
  ...
  return (raw[FILTER_FIELDS[type]] ?? []).some((entry) =>
    sameText(entry, value),
  );
}

function matchesSearch(
  raw: RawRecipeDetail,
  { query, filters }: RecipesQuery,
): boolean {
...
export async function listRecipes(search: RecipesQuery): Promise<RecipeList> {
  ...
  if (search.ids?.length) {
    ...
    const recipes = all
      .filter((raw) => wanted.has(String(raw.id)))
      .map(getRecipeSummary);
...
export async function findRecipe(id: string): Promise<RecipeDetail | null> {
  const raw = (await getAllRecipes()).find(
...
export async function getFilterGroups({
  query,
  filters,
}: RecipesQuery): Promise<FilterGroup[]> {
  const all = await getAllRecipes();
  ...
  return FILTER_TYPES.map((type) => {
    ...
    const reachable = all.filter((raw) =>
      matchesSearch(raw, { query, filters: others }),
    );
