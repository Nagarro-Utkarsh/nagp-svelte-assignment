...
import type { RawIngredient, RawRecipeDetail, RawRecipeSummary } from './spoonacular.types';
...
function getInstructions(raw: RawRecipeDetail): string[] {
	const steps = (raw.analyzedInstructions ?? []).flatMap((group) => group.steps ?? []);

	if (steps.length) {
		return steps.map((step) => step.step?.trim()).filter((step): step is string => Boolean(step));
	}
...
function getTags(raw: RawRecipeDetail): string[] {
	...
	return [...new Map(tags.map((tag) => [tag.toLowerCase(), tag])).values()].slice(0, TAG_LIMIT);
}
